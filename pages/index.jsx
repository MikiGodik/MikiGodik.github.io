...
/** Calculates total years of Nitro delivered: starts at 80, grows by a pseudo-random 0.1–0.2 per day since 2025-01-01 GMT+1 */
function useTotalNitros() {
  const [count, setCount] = useState(80)

  useEffect(() => {
    const BASE = 80
    const BASE_DATE = new Date('2026-04-10T00:00:00+01:00')
    const nowGMT1 = new Date(
      new Date().toLocaleString('en-US', { timeZone: 'Europe/Paris' })
    )
    const daysDiff = Math.max(0, Math.floor((nowGMT1 - BASE_DATE) / 86400000))

    // Deterministic "random" daily increment between 0.1 and 0.2, seeded by day index
    let total = BASE
    for (let d = 0; d < daysDiff; d++) {
      const pseudoRandom = Math.abs(Math.sin(d * 12.9898) % 1)
      const increment = 0.1 + pseudoRandom * 0.1
      total += increment
    }
    ...
    <nav ref={navRef} className="nav" id="main-nav">
      <div className="nav-inner">
        <a className="nav-logo" href="#">
          <img src="/Logo.png" alt="Nitro Vault" style={{ height: 30, width: 'auto' }} />
        </a>
    ...
    <section className="hero" id="hero">
      <div className="container">
        ...
        <h1 className="t-display hero-headline">
          <span className="count-num">{count.toFixed(1)}</span> Years of Nitro
          <br />Delivered
        </h1>
...
function TrustBar() {
  const badges = [
    { icon: 'fa-solid fa-lock',           label: 'No passwords needed' },
  ...
  return (
    <section className="trust-section">
      <div className="container">
        <h2 className="t-title">Honest pricing.<br />Same, but without the markup.</h2>
